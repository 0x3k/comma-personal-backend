...
// pilotAuthMockDB dispatches QueryRow between lookup (GetDeviceByPublicKey
// -> SELECT) and insert (CreateDevice -> INSERT) based on the SQL text so we
// can exercise both paths in one test. The generic mockDBTX in helpers_test.go
// handles simpler single-query tests.
type pilotAuthMockDB struct {
	// existingDevice is returned from SELECT lookups. If nil, SELECT yields
	// pgx.ErrNoRows.
