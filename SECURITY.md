...
- Device authentication uses per-device public-key JWTs (RS256 or ES256). Each device signs its own tokens with the private key generated during registration; the backend verifies them against the public key it stored at pilotauth time. There is no shared server secret.
